...
function processWeatherData(weatherData) {
  const publishingOffice = weatherData[1].publishingOffice;
  const areaName = weatherData[1].timeSeries[0].areas[0].area.name;
  document.getElementById("location").textContent = `${publishingOffice}: ${areaName}`;

  const weatherElements = document.querySelectorAll('.weather');
  weatherElements.forEach((element, index) => {
    if (index >= weatherData[1].timeSeries[0].timeDefines.length) return;

    const weatherCodeValue = weatherData[1].timeSeries[0].areas[0].weatherCodes[index];
    const date = new Date(weatherData[1].timeSeries[0].timeDefines[index]);
    const day = dayList[date.getDay()];
    const month = date.getMonth() + 1;
    const dayOfMonth = date.getDate();

    const weatherImg = element.querySelector('.weatherImg');
    const weatherTelop = element.querySelector('.weatherTelop');
    const tempMin = element.querySelector('.tempMin');
    const tempMax = element.querySelector('.tempMax');

    element.querySelector('.date').textContent = `${month}/${dayOfMonth}(${day})`;
    weatherImg.src = `https://www.jma.go.jp/bosai/forecast/img/${weatherCode[weatherCodeValue][0]}`;
    weatherImg.alt = weatherCode[weatherCodeValue][2];
    weatherTelop.textContent = weatherCode[weatherCodeValue][2];
    tempMin.textContent = weatherData[1].timeSeries[1].areas[0].tempsMin[index] + '℃';
    tempMax.textContent = weatherData[1].timeSeries[1].areas[0].tempsMax[index] + '℃';
  });
}
